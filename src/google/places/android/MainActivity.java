...
public class MainActivity extends SherlockActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!isNetworkAvailable()) {
			Toast.makeText(getApplicationContext(),
					"Enable internet connection and RE-LAUNCH!!",
		...
		ArrayAdapter<CharSequence> list = ArrayAdapter.createFromResource(
				context, R.array.places, R.layout.sherlock_spinner_item);
	...
	public boolean onNavigationItemSelected(int itemPosition, long itemId) {
		...
		new readFromGooglePlaceAPI()
				.execute("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=your_lat,your_long&radius=100000&sensor=true&key=your_key_here&types="
						+ places[itemPosition]);
		...
		protected void onPostExecute(String str) {
			...
			try {
				...
				for (int i = 0; i < results.length(); i++) {
					...
					addValues.setRating(arrayItems.getString("rating")
							.toString());
					addValues.setVicinity(arrayItems.getString("vicinity")
							.toString());
